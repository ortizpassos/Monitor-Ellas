...
mongoose.connect('mongodb://localhost:27017/monitor-ellas')
  .then(async () => {
    ...
    // Busca dispositivos e funcionários existentes
    const dispositivos = await Dispositivo.find().limit(3);
    const funcionarios = await Funcionario.find().limit(3);
    
    if (dispositivos.length === 0 || funcionarios.length === 0) {
      console.log('Não há dispositivos ou funcionários cadastrados. Execute o sistema primeiro.');
      process.exit(0);
    }
